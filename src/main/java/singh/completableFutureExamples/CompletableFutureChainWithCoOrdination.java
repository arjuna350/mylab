 ...
 *
 */
public class CompletableFutureChainWithCoOrdination {
    Object obj = new Object();
	DqExecutor executor = new DqExecutor();
	EnhancedDqResultMonitor monitor = new EnhancedDqResultMonitor(obj);
	DqResultUploder uploder = new DqResultUploder();
	
	ExecutorService baseExecutor = Executors.newFixedThreadPool(5);
	ExecutorService monitorExecutor = Executors.newFixedThreadPool(2);
	
	Function<String, String> executorFunction = profileId->executor.executeDq(profileId);
	Function<String, String> monitorFunction =  profileId->{
		try {
			return monitor.resultMonitor(profileId);
	...
	public static void main(String[] args) {
		CompletableFutureChainWithCoOrdination example = new  CompletableFutureChainWithCoOrdination();
